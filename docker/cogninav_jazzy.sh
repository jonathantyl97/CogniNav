#!/usr/bin/env bash
# Persistent ROS 2 Jazzy dev container for CogniNav (ORB-SLAM3 + Iridescence viz).
# Based on ~/jazzy.sh but keeps the container and mounts the CogniNav workspace.

set -euo pipefail

CONTAINER_NAME="ros2_jazzy_cogninav"
...
if docker ps -a --format '{{.Names}}' | grep -qx "$CONTAINER_NAME"; then
  ...
  docker run -it \
    ...
    }; source /opt/ros/jazzy/setup.bash; cd /root/cogninav; exec bash'
fi
